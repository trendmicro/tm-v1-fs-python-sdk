...
def test_init_by_region(utilmock):
    amaas.grpc.aio.init_by_region("au-1", "dummy_key")

    # ensure True is passed to is_aio_channel parameter
    utilmock.assert_called_with('au-1', 'dummy_key', True, None, True)
...
def test_init(utilmock):
    amaas.grpc.aio.init("au-1", "dummy_key")

    # ensure False is passed to is_aio_channel parameter
    utilmock.assert_called_with('au-1', 'dummy_key', False, None, True)
...
@pytest.mark.parametrize("error_type, expected_exception", [
        ...
        ( MockScanServicer.IDENTIFIER_MISMATCHED, [ AMaasErrorCode.MSG_ID_ERR_UNEXPECTED_CMD_AND_STAGE, amaas.grpc.scan_pb2.CMD_QUIT, amaas.grpc.scan_pb2.STAGE_RUN ] ),
