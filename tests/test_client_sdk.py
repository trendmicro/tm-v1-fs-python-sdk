...
def test_init_by_region(utilmock):
    amaas.grpc.init_by_region("au-1", "dummy_key")

    # ensure False is passed to is_aio_channel parameter
    utilmock.assert_called_with('au-1', 'dummy_key', True, None, False)


@patch("amaas.grpc._init_util")
def test_init(utilmock):
    amaas.grpc.init("au-1", "dummy_key")

    # ensure False is passed to is_aio_channel parameter
    utilmock.assert_called_with('au-1', 'dummy_key', False, None, False)
...
def run_setup():
    ...
    amaas.grpc.protos.scan_pb2_grpc.add_ScanServicer_to_server(MockScanServicer(), server)
...
#
# This routine is to create a data file for testing. The binary file contains of NUM_DATA_LOOP of consecutive bytes from 0x00 to 
# 0xff. This is to make checking the correctness of the content of the retrieval easier.
# The grpc client receives a sequence of instructions from grpc server to retrieve chunks of data from a data file during 
# scanning. One of the important checking is to ensure the data retrived is what is actually rqeuested by the server.
#
...
def test_scan_file_failed_authentication():
    handle = amaas.grpc.init_by_region("us-1", "api-key", True, None)
    with pytest.raises(AMaasException) as exc_info:
        amaas.grpc.scan_file(handle, "fake_server_cert.pem")
...
def test_generate_message_init():
    ...
    with open(TEST_DATA_FILE_NAME, "rb") as f:
        ...
        server_resp = amaas.grpc.scan_pb2.C2S(stage=amaas.grpc.scan_pb2.STAGE_INIT,
        ...
        c2s_msg = next(amaas.grpc._generate_messages(pipeline, f, stats))
...
#
# Testing the generate_message method correctly processing a number of back and forth data retrieval message from grpc server.
#
def test_generate_message_data():
    ...
    for cnt in range(5):
        ...
        server_resp = amaas.grpc.scan_pb2.S2C(stage=amaas.grpc.scan_pb2.STAGE_RUN,
        ...
        assert c2s_msg.offset == end
        verify_buf_with_data(c2s_msg.chunk, start)


#
# Testing the generate_message method correctly raise exception when recieved wrong command in STAGE RUN from server.
#
def test_generate_message_data_wrong_cmd_exception():
    ...
    with open(TEST_DATA_FILE_NAME, "rb") as f:
        ...
        with pytest.raises(AMaasException) as exc_info:
            c2s_msg = next(amaas.grpc._generate_messages(pipeline, f, stats))
        assert exc_info.value.args[0] == AMaasErrorCode.MSG_ID_ERR_UNEXPECTED_CMD_AND_STAGE
...
def test_generate_message_final():
    ...
    stats = {}
    f = open(TEST_DATA_FILE_NAME, "rb")
    server_resp = amaas.grpc.scan_pb2.S2C(stage=amaas.grpc.scan_pb2.STAGE_FINI,
                               cmd=amaas.grpc.scan_pb2.CMD_QUIT)

    pipeline.set_message(server_resp)
    client_msg = next(amaas.grpc._generate_messages(pipeline, f, stats), None)
    assert client_msg == None
...
def test_generate_message_unknwon_stage():
    ...
    f = open(TEST_DATA_FILE_NAME, "rb")
    UNKNOWN_STAGE = 9999
    server_resp = amaas.grpc.scan_pb2.S2C(stage=UNKNOWN_STAGE,
                               cmd=amaas.grpc.scan_pb2.CMD_QUIT)
...
def test_scan_file_success():
    ...
    response = amaas.grpc.scan_file(handle, 'fake_server_cert.pem')
...
def test_scan_file_not_found():
    ...
    with pytest.raises(AMaasException) as exc_info:
        response = amaas.grpc.scan_file(handle, NOT_EXIST_FILE)
...
def test_scan_file_no_permission():
    ...
    os.chmod(NOT_PERMISSION_FILE, 0o111)
    with pytest.raises(AMaasException) as exc_info:
        response = amaas.grpc.scan_file(handle, NOT_PERMISSION_FILE)
...
def test_scan_buffer_virus():
    ...
    with open(TEST_DATA_FILE_NAME, mode="rb") as bfile:
        buffer = bfile.read()
        response = amaas.grpc.scan_buffer(handle, buffer, MockScanServicer.IDENTIFIER_VIRUS)
...
@pytest.mark.parametrize("error_type, expected_exception", [ 
        ( MockScanServicer.IDENTIFIER_EXCEED_RATE, [ AMaasErrorCode.MSG_ID_ERR_RATE_LIMIT_EXCEEDED ] ),
        ( MockScanServicer.IDENTIFIER_UNKNOWN_CMD, [ AMaasErrorCode.MSG_ID_ERR_UNKNOWN_CMD, MockScanServicer.UNKNOWN_CMD ] ),
        ( MockScanServicer.IDENTIFIER_GRPC_ERROR, [ AMaasErrorCode.MSG_ID_GRPC_ERROR, grpc.StatusCode.INTERNAL.value[0] ] )
      ] 
    )
def test_scan_buffer_exceptions(error_type, expected_exception):
    ...
    with open(TEST_DATA_FILE_NAME, mode="rb") as bfile:
        buffer = bfile.read()
        with pytest.raises(AMaasException) as exc_info:
            response = amaas.grpc.scan_buffer(handle, buffer, error_type)
